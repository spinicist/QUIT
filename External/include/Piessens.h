 ...
 */
template <typename Scalar> class Piessens {
  public:
    /**
    ...
    static void kronrod(unsigned int                             nNodes,
        ...
        for (size_t k = 1; k < m; ++k) {
            ...
            tau(k) = ((aK - Scalar(1.0)) * aK - aN * (aN + Scalar(1.0))) * (aK + Scalar(2.0)) *
                     tau(k - 1) /
            ...
            for (size_t i = 1; i <= k; ++i) {
                betaCoeffs(m - k - 1) =
                    betaCoeffs(m - k + i - 1) * tau(i - 1) + betaCoeffs(m - k - 1);
            }
        }
        ...
        // @TODO The usage of constant Pi with fixed precision needs to be changed to the following
        // for multiprecision
        ...
        Scalar chebCoeff1 = Scalar(1.0) - Scalar(1.0) / (Scalar(8.0) * aN * aN) +
                            Scalar(1.0) / (Scalar(8.0) * aN * aN * aN);
        ...
        for (size_t k = 0; k < nNodes; ++k) {
            abscWeightKronrod(nNodes, m, even, chebCoeff2, betaCoeffs, abscK,
                              weightGaussKronrod(k));
     ...
     *
     *  Input/output:
     * \param[in,out] abscGaussKronrod An estimate for the abscissa on input and the computed
     * abscissa on output. \param[in,out] weightGaussKronrod The Gauss-Kronrod weight.
     */
    static void abscWeightKronrod(unsigned int nNodes, unsigned int m, bool even, Scalar chebCoeff,
                                  Eigen::Array<Scalar, Eigen::Dynamic, 1> betaCoeffs,
                                  Scalar &abscGaussKronrod, Scalar &weightGaussKronrod) {
        Scalar ai;
        ...
        while (abs(delta) > machineEpsilon()) {
            ...
            if (iter > iterationLimit) {
                std::cout << "@abscWeightKronrod Newton-Raphson iterative abscissae solver failed."
                          << std::endl;
                return;
            }
        }
     ...
     */
    static void abscWeightGauss(unsigned int nNodes, unsigned int m, bool even,
        ...
        while (abs(delta) > machineEpsilon()) {
            ...
            if (nNodes <= 1) {
                if (machineEpsilon() < abs(abscGaussKronrod)) {
                    p2 = (Scalar(3.0) * (abscGaussKronrod) * (abscGaussKronrod)-Scalar(1.0)) /
                         Scalar(2.0);
            ...
            for (size_t k = 0; k < nNodes - 1; ++k) {
                ai = ai + Scalar(1.0);
                p2 = ((ai + ai + Scalar(1.0)) * abscGaussKronrod * p1 - ai * p0) /
                     (ai + Scalar(1.0));
                pd2 = ((ai + ai + Scalar(1.0)) * (p1 + abscGaussKronrod * pd1) - ai * pd0) /
                      (ai + Scalar(1.0));
                p0  = p1;
                p1  = p2;
                pd0 = pd1;
            ...
            if (iter > iterationLimit) {
                std::cout << "@abscWeightGauss Newton-Raphson iterative abscissae solver failed."
                          << std::endl;
                ;
                return;
            }
     ...
     */
    static void
    computeAbscissaeAndWeights(unsigned int                             nNodes,
                               Eigen::Array<Scalar, Eigen::Dynamic, 1> &abscGaussKronrod,
                               Eigen::Array<Scalar, Eigen::Dynamic, 1> &weightGaussKronrod,
