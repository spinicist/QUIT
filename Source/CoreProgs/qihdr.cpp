...
int main(int argc, char **argv) {
    ...
    for (const std::string& fname : QI::CheckList(filenames)) {
        ...
        if (print_all || verbose) std::cout << "Dimension:  ";
        if (print_all || print_dims) std::cout << dims << std::endl;
        ...
        if (print_all || print_spacing) {
            if (print_spacing.Get() > dims) {
                ...
                if (print_all || verbose) std::cout << "Spacing:    ";
                if (print_all || print_spacing) { for (size_t i = start_dim; i < end_dim; i++) std::cout << imageIO->GetSpacing(i) << "\t"; std::cout << std::endl; }
            }
        }
        if (print_all || verbose) std::cout << "Origin:     ";
        if (print_all || print_origin)   { for (size_t i = 0; i < dims; i++) std::cout << imageIO->GetOrigin(i) << "\t"; std::cout << std::endl; }
        ...
        if (print_all || verbose) std::cout << "Voxel vol:  ";
        if (print_all || print_voxvol)   { double vol = imageIO->GetSpacing(0); for (size_t i = 1; i < dims; i++) vol *= imageIO->GetSpacing(i); std::cout << vol << std::endl; }
