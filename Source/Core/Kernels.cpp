...
double TukeyKernel::value(const Eigen::Array3d &pos,
    ...
    const double v =
        (r <= (1 - m_a)) ? 1 : 0.5 * ((1 + m_q) + (1 - m_q) * cos(M_PI * (r - (1 - m_a)) / m_a));
    return v;
}
...
RectKernel::RectKernel(std::istream &istr) {
    if (!istr.eof()) {
        ...
        std::getline(istr, nextValue, ',');
        m_width = stoi(nextValue);
        std::getline(istr, nextValue, ',');
        m_val_inside = stod(nextValue);
        std::getline(istr, nextValue);
        m_val_outside = stod(nextValue);
    }
    if (m_dim > 2) {
        QI::Fail("Dimension for Rectangular filter must be less than 3");
    }
}
...
FixFSEKernel::FixFSEKernel(std::istream &istr) {
    if (!istr.eof()) {
        ...
        m_dim = stoi(nextValue);
        std::getline(istr, nextValue, ',');
        m_etl = stoi(nextValue);
        std::getline(istr, nextValue, ',');
        m_kzero = stoi(nextValue);
        std::getline(istr, nextValue, ',');
        m_te1 = stod(nextValue);
        std::getline(istr, nextValue, ',');
        m_esp = stod(nextValue);
        std::getline(istr, nextValue);
        m_T2 = stod(nextValue);
    }
    if (m_dim > 2) {
        QI::Fail("Dimension for FixFSE filter must be less than 3");
    }
}
