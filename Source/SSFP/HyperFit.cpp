...
void SemiaxesToHoff(
    const double A, const double B, const double c, double &G, double &a, double &b) {
...
Eigen::MatrixXd HyperC(const Eigen::ArrayXd &x, const Eigen::ArrayXd &y) {
    ...
    C << 6 * sx, 6 * xy, sx + sy, 6 * xc, 2 * yc, 1, 6 * xy, 4 * (sx + sy), 6 * xy, 4 * yc, 4 * xc,
        0, sx + sy, 6 * xy, 6 * sy, 2 * xc, 6 * yc, 1, 6 * xc, 4 * yc, 2 * xc, 4, 0, 0, 2 * yc,
        4 * xc, 6 * yc, 0, 4, 0, 1, 0, 1, 0, 0, 0;
    return C;
}

QI::FitReturnType HyperFit::fit(const std::vector<Eigen::ArrayXcd> &inputs,
                                const Eigen::ArrayXd & /* Unused */,
                                QI_ARRAYN(OutputType, EllipseModel::NV) & outputs,
                                ResidualType & /*Unused*/,
    ...
    const double num = 2 * (Z[0] * (Z[4] * Z[4]) + Z[2] * (Z[3] * Z[3]) + Z[5] * (Z[1] * Z[1]) -
                            2 * Z[1] * Z[3] * Z[4] - Z[0] * Z[2] * Z[5]);
    double       A =
        sqrt(num / (dsc * (sqrt((Z[0] - Z[2]) * (Z[0] - Z[2]) + 4 * Z[1] * Z[1]) - (Z[0] + Z[2]))));
    double B = sqrt(
        num / (dsc * (-sqrt((Z[0] - Z[2]) * (Z[0] - Z[2]) + 4 * Z[1] * Z[1]) - (Z[0] + Z[2]))));
    if (A > B) {
        std::swap(A, B);
    }
    double G, a, b;
