...
struct PrepQMTRxFull : Model<double, double, 7, 1, 1, 0, RealNoise<double>> {
    static int const       NS = 1; // Number of parameters that need to be scaled
    PrepZTESequence const &sequence;
    RegularGrid const     &A_sl;
    // Pools are "free" and "semi-solid"
    std::array<std::string, NV> const varying_names{
        "M0", "T1_f", "T1_s", "T2_s", "f_s", "R_x", "B1"};
    VaryingArray const start{1.0, 2.0, 0.35, 14e-6, 0.1, 14.0, 1.0};
    VaryingArray const lo{0.01, 0.5, 0.05, 10e-6, 0.01, 1.0, 0.5};
    VaryingArray const hi{100.0, 5.0, 0.5, 20e-6, 0.5, 50.0, 1.5};
