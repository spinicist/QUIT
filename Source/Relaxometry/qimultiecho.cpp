...
int main(int argc, char **argv) {
    ...
    args::ValueFlag<std::string> file(parser, "FILE", "Read JSON input from file instead of stdin", {"file"});
    ...
    if (simulate) {
        ...
        if (subregion) fit->SetSubregion(QI::RegionArg(subregion.Get()));
        ...
        for (int i = 0; i < me->n_outputs(); i++) {
            QI::WriteVectorImage(fit->GetOutput(i), outPrefix + me->model.varying_names.at(i) + QI::OutExt());
        }
