...
std::array<const std::string, ThreePoolModel::NV> ThreePoolModel::varying_names{
    {"PD"s, "T1_m"s, "T2_m"s, "T1_ie"s, "T2_ie"s, "T1_csf"s, "T2_csf"s, "tau_m"s, "f_m"s,
     "f_csf"s}};
...
auto ThreePoolModel::signal(const Eigen::ArrayXd &  varying, const QI_ARRAYN(double, NF) & fixed,
...
Eigen::ArrayXd ThreePoolModel::SSFP1(const double &PD, const double &T1, const double &T2,
                                     const double &f0, const double &B1,
...
Eigen::ArrayXd ThreePoolModel::ssfp_signal(const Eigen::ArrayXd &v,
    ...
    Eigen::VectorXd m_ab = two_pool.ssfp_signal(two_pool_varying, fixed, s);
    Eigen::VectorXd m_c =
        SSFP1(v[0] * v[9], v[5], v[6], fixed[0], fixed[1], s) * sqrt(exp(-s->TR / v[6]));
    Eigen::VectorXd signal = m_ab + m_c;
    if (scale_to_mean) {
        signal /= signal.mean();
    }
