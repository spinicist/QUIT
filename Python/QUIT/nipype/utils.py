...
class QiRFprofileInputSpec(QUITCommandInputSpec):
    ...
    # Outputs
    out_file = File(exists=False, argstr='%s', mandatory=True, desc='Input B1+ file', position=1)
...
class QiRFprofile(QUITCommand):
    ...
    def _list_outputs(self):
        ...
        outputs['out_file'] = os.path.abspath(self._add_prefix(self.inputs.out_file))
...
class QiAffineInputSpec(QUITCommandInputSpec):
    ...
    # Options
    xfm_file = File(exists=False, argstr='--tfm=%s', mandatory=True, desc='Write out the transformation to a file')
    offX = traits.Int(desc='Translate origin in X direction', argstr='-offX=%d')
    offY = traits.Int(desc='Translate origin in Y direction', argstr='-offY=%d')
    offZ = traits.Int(desc='Translate origin in Z direction', argstr='-offZ=%d')
    rotX = traits.Int(desc='Rotate about X-axis by angle (degrees)', argstr='-rotX=%d')
    rotY = traits.Int(desc='Rotate about Y-axis by angle (degrees)', argstr='-rotY=%d')
    rotZ = traits.Int(desc='Rotate about Z-axis by angle (degrees)', argstr='-rotZ=%d')
    scale = traits.Float(desc='Scale by a constant', argstr='--scale=%f')
    permute = traits.String(desc='Permute axes, e.g. 2,0,1. Negative values mean flip as well', argstr='--permute=%s')
    flip = traits.String(desc='Flip an axis, e.g. 0,1,0. Occurs AFTER any permutation.', argstr='--flip=%s')

    # This should probably be enum instead
    center = traits.String(desc='Set the origin to geometric center (geo) or (cog)', argstr='--center=%s')
...
class QiMaskInputSpec(QUITCommandInputSpec): 
    # Inputs
    in_file = File(exists=True, argstr='%s', mandatory=True, position=0, desc='Input File')
    ...
    vol = traits.Int(desc='Choose volume to mask in multi-volume file. Default 1, -1 selects last volume', 
                    argstr='--volume=%d')
    complex_data = traits.Bool(desc='Input data is complex, take magnitude first', argstr='--complex')
    ...
    rats = traits.Float(desc="Perform the RATS step, argument is size threshold for connected component",
                    argstr='--rats=%f')
    fill_holes = traits.Int(desc="Fill holes in thresholded mask with radius N", argstr='--fillh=%d')
