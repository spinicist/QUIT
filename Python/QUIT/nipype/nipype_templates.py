...
class QiXXInputSpec(QUITCommandInputSpec):
    ...
    param_file = File(desc='.json file', position=-1, argstr='< %s', 
...
file_Trait = File(desc='Description', argstr='-X=%s', exists=True)      # Use exist=True if input. False if output

enum_trait = traits.Enum("Opt1","Opt2","Opt3", desc="Description", argstr="-X=%d")

flot_trait = traits.Float(desc='Description', argstr='-X=%f')

dict_trait = traits.Dict(desc='Description', position=1, argstr='', mandatory=True)
